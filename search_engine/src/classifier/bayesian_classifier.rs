...
impl<'a> BayesianClassifier<'a> {
    ...
    fn calculate_probability_term_given_class(&self, doc_id: u32, index_for_class: &DiskInvertedIndex, discriminating_vocab: &Vec<String>) -> f64 {
        ...
        for term in discriminating_vocab {
            discriminating_vocab_hashset.insert(term.clone());
        }
        ...
        for discriminating_term in discriminating_vocab {
            discriminating_terms_score += index_for_class.get_term_frequency(discriminating_term);
        }
...
impl<'a> Classifier<'a> for BayesianClassifier<'a> {
    fn classify(&self, doc_id: u32) -> &'a str {
        let discriminating_vocab = self.build_discriminating_vocab_set(50);
